    ...
    switch (req.method) {
      case "POST": // Used to create a new invite link
        await new Promise((resolve) => {
          ...
          connection.query(
            ...
            [req.body.leagueID, session.user.id],
                ...
                connection.query(
                  ...
                  function (error, result, fields) {
                    ...
                    if (error === null) {
                      ...
                      console.warn(
                        `League ${req.body.leagueID} tried to create invite link`
                      );
      ...
      case "GET": // Used to get a list of invite links for a league
        await new Promise(() => {
          connection.query(
            "SELECT * FROM leagueUsers WHERE leagueID=? and user=?",
            [req.query.leagueID, session.user.id],
            function (error, results, fields) {
              if (results.length > 0) {
                connection.query(
        ...
        await new Promise(() => {
          connection.query(
            ...
            function (error, results, fields) {
              if (results.length > 0) {
                connection.query(
                  "DELETE FROM invite WHERE leagueID=? and inviteID=?",
                  [req.body.leagueID, req.body.link]
                );
                console.log(
