              ...
              function (error, result2, fields) {
                if (result2.length > 0) {
                    ...
                    function (error, result3, field) {
                      ...
                      if (result3.length == 0) {
                        connection.query(
                          ...
                          [result.leagueID, session.user.id, result.leagueID]
                        );
                          ...
                          function (error, point, fields) {
                            ...
                            while (matchday > 0) {
                              connection.query(
                                ...
                                [result.leagueID, session.user.id, matchday]
                              );
                              matchday--;
                            }
                            console.log(
                              `Player ${session.user.id} joined league ${result.leagueID}`
                            );
                            resolve();
                          }
                        );
                  ...
                  res
                    .status(500)
                    .end(
                      "An error occured please contact the website administrator"
                    );
                  resolve();
                }
              }
