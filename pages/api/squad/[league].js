            ...
            case "POST":
                ...
                function positionOpen(limit, position) {
                    return new Promise((resolve, reject) => {
                        connection.query(`SELECT * FROM squad WHERE position='${position}'`, function(error, result, fields) {
                ...
                if (formation !== undefined) {
                    ...
                    let included = false
                    validFormations.forEach((e) => {if (JSON.stringify(e) == JSON.stringify(formation)) {included = true}})
                    if (included) {
                                ...
                                connection.query("SELECT * FROM squad WHERE leagueID=? and user=? and playeruid=?", [league, user, e], function(error, result, field) {
                                    ...
                                    if (result.length > 0) {
                                        ...
                                        if (position === "bench") {
                                            ...
                                            connection.query("SELECT position, locked FROM players WHERE uid=?", [e], function(error, result, field) {
                                                if (result[0].locked) {
                                                    reject(`${e} is locked`)
                                                } else {
